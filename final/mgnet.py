...
class MgNet(nn.Module):
    def __init__(self, args,num_classes):
        ...
        self.conv1 = nn.Conv2d(self.num_channel_input, args.channel, kernel_size=3, stride=1,\
                               padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(args.channel)        

        A_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3, stride=1, padding=1, bias=False)
        if not args.wise_B:
            B_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3,stride=1, padding=1, bias=False)

        layers = []
        for l, num_iteration_l in enumerate(self.num_iteration):
            for i in range(num_iteration_l):
                if args.wise_B:
                    B_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3,\
            ...
            if l < len(self.num_iteration)-1:
                A_old = A_conv
                #B_old = B_conv
                A_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3,\
                                   stride=1, padding=1, bias=False)
                if not args.wise_B:
                    B_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3,\
                                       stride=1, padding=1, bias=False)
                Pi_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3,\
                                    stride=2, padding=1, bias=False)
                R_conv = nn.Conv2d(args.channel, args.channel, kernel_size=3, \
                                   stride=2, padding=1, bias=False)
                layers= [MgRestriction(A_old, A_conv, Pi_conv, R_conv)]
                
        
        self.pooling = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(args.channel ,num_classes)
