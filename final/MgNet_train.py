...
def load_optimizer(args, model, testfreq):
    if args.optim == "ssm":
       return SSM(model.parameters(), lr=args.lr, weight_decay=args.wd, momentum=args.momentum, dampening=args.dampening, testfreq=testfreq, var_mode=args.vm, mode=args.km,
                  leak_ratio=args.lk, minN_stats=args.minstat, samplefreq=args.sf, significance=args.sig, drop_factor=args.drop, trun=args.trun)
    if args.optim == "sasa+":
...
if __name__ == "__main__":
    ...
    if args.optim == "sgd":
        ...
        name = "SSM_mgnet{}_trun{}_sig{}_lk{}_min{}_sf{}_drop{}_{}_".format(args.channel, str(args.trun)[-1], str(args.sig)[-1], args.lk, args.minstat, args.sf, args.drop, args.km)
